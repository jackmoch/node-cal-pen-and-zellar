...
const { centerString, 
				trim, 
				newLine } = require('../lib/render')
const { createYearHeader } = require('../lib/year')
...
describe('year', () => {
	describe('createYearHeader', () => {
		...
		it('should write out a header witih the year', () => {
			const expected = '                             2016                               '
			const yearString = centerString('2016', 64)
			assert.equal(createYearHeader(yearString), expected)
		})
